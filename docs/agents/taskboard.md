...
| STATE-FE-001 | Frontend | done | Preserve extraction state across background/auth lifecycle and track download/save failures. | `wechat-miniprogram/pages/index/index.js`, `docs/agents/handoffs/2026-06-21-frontend-lifecycle-download-state.md` | Frontend commit `0808fbe` removes index-side token reverify after auth success, preserves scanner state across foreground return, tracks per-result save state, surfaces all-download partial failures, preserves unsaved/failed/unknown groups for `新任务`, and makes completed-state single-image delete remove only that source and its results. |
| STATE-REV-001 | Reviewer | done | Review `STATE-FE-001` after Frontend handoff is available. | Review only; `docs/agents/handoffs/2026-06-21-reviewer-lifecycle-download-state.md` | Reviewer commit `f10daa1` verdict: approved. Reviewer confirmed no index-side token reverify, missing-token/settings-clear behavior, lifecycle state preservation, per-result save-state tracking, visible download/save partial failures, `新任务` preservation/remapping, completed-state single-image delete behavior, picker compatibility, and no changes to Backend routes or RunPod behavior. |
| RESULTDL-FE-001 | Frontend | pending | Preserve immediate result download/display while adapting to 1200-width outputs and upload retry/text changes. | `wechat-miniprogram/pages/index/index.js`, `wechat-miniprogram/pages/index/index.wxml`, `wechat-miniprogram/pages/index/index.wxss` if needed, `docs/agents/handoffs/2026-06-21-frontend-large-result-downloads.md` | Must keep the current product behavior that every Backend-returned extracted polaroid is downloaded immediately when received and displayed immediately in the extraction result area. Do not implement lazy result download, LRU eviction, delayed display, or a policy that discards completed-size downloaded result images while the mini program remains open. Completed-size result images must remain available in Frontend state/storage until the user exits the mini program or explicitly deletes that result through `新任务` cleanup or source-image deletion. After Backend reduces output size, Frontend must continue handling immediate display, all-download, single-result save, per-result save state, and completed-state navigation without deleting results. Set upload timeout max retries to 3. In the index empty-preview area, under `选择或添加最多9张图片`, add a second helper line exactly `单次处理的拍立得数量不应超过50张`. Preserve `新任务` preservation rules, completed-state single-image delete behavior, batch ordering, source-image grouping, picker behavior, auth policy, upload/process/status/cancel contracts, `postprocess_mode`, `polaroid_size`, and main-sync routes. Verify that after 40+ immediately downloaded/displayed results, later result taps and a new image upload still work under this retained-image behavior. |
| RESULTDL-BE-001 | Backend | pending | Reduce output size to 1200-width mini while preserving immediate incremental result availability. | `backend/app.py`, `scripts/check_polaroid_size.py` or focused equivalent, `docs/agents/handoffs/2026-06-21-backend-large-result-downloads.md` | Change extracted polaroid completed output from mini width `1600` to mini width `1200` while preserving the existing `1:1.59` mini ratio and mini/wide/auto semantics. Wide output should remain equivalent to two mini outputs side by side, so its width scales from `3200` to `2400`; height and image-area vertices must be recomputed consistently with the existing mini/wide border proportions. Backend must preserve immediate incremental availability: after each polaroid is extracted, that result must be appended and exposed through status/result routes immediately, not delayed until the source image or batch completes. Verify `/api/status/<task_id>`, `/api/result/<task_id>/<result_id>`, and subsequent `/api/process` uploads remain valid after high-count result downloads, including result ids 0, 39, 40, and 59 or similar, followed by a fresh upload/process smoke. Do not change RunPod startup, SAM detection, auth, frontend field names, postprocessing modes, mini/wide/auto request semantics, upload cancel, task cancel, contact routes, or the production pod ID token flow. |
| RESULTDL-REV-001 | Reviewer | pending | Review exact RESULTDL behavior after Frontend and Backend handoffs are available. | Review only; `docs/agents/handoffs/2026-06-21-reviewer-large-result-downloads.md` | Reviewer must inspect Frontend and Backend diffs/handoffs and verify the exact user-directed constraints: Backend mini output width is 1200, wide output scales consistently, mini/wide/auto still work, each extracted polaroid is made available immediately after extraction, Frontend downloads each returned result immediately, displays it immediately in the extraction results area, and retains completed-size downloaded images until mini-program exit or explicit user deletion through `新任务`/source-image deletion. Reviewer must reject lazy download, LRU eviction, delayed result display, or cleanup that discards completed result images during the active mini-program session. Verify upload timeout max retries is 3 and the empty-preview helper shows both `选择或添加最多9张图片` and `单次处理的拍立得数量不应超过50张`. Also verify the clarified 40+ flow: after about 40 immediate downloads, later result downloads still work and a new image can still be selected/uploaded/processed. Confirm no regressions to lifecycle state preservation, per-result save tracking, `新任务` preservation, completed-state single-image delete, picker, auth policy, batch processing, upload/status/cancel, postprocessing, size selection, custom tab bar, calendar, and `izaya7-map`. Reviewer should run `node --check`, Backend compile/focused smoke, `git diff --check`, frontend mocks for immediate result display/retention and post-download upload, and backend geometry/result-route plus fresh-upload smoke. |
