...
Fix the extracted-result save-state refresh regression found after the approved save-state work reached `main`: receiving later results or refreshing extraction status must never downgrade a result from album-written or downloaded state back to remote-only.
...
| Frontend | `C:\Users\20888\Desktop\chekinana-frontend` | `codex/frontend-next` | Own `SAVE-FE-002`: preserve result save/download state across extraction refreshes |
...
| Reviewer | `C:\Users\20888\Desktop\chekinana-reviewer` | `codex/reviewer-next` | Own `SAVE-REV-002` after Frontend handoff |
...
| SAVE-FE-002 | Frontend | pending | Preserve result download/save state across status refreshes and newly received extraction results. | `wechat-miniprogram/pages/index/index.js`, focused mock/test if present, `docs/agents/handoffs/2026-06-23-frontend-save-state-refresh.md` | Reproduce and fix the user-reported regression where each newly received extracted result refreshes the result list and resets prior result badges from green downloaded or green-check album to yellow remote-only. Frontend must treat album-written state as a durable, irreversible local truth for the active result set; no status poll, `normalizeImages`, `mergeImages`, `updatePartialImages`, `finishWithImages`, `finishWithShortage`, `finishBatchExtract`, `pollBatchTask`, background predownload callback, or closure-held `baseImages`/`collectedImages` array may downgrade `saveStatus=saved` or the green-check `album` badge. Downloaded local-path state must also survive refreshes when the local path is known, so green downloaded dots do not reset to yellow and trigger unnecessary redownload. Audit all direct `setData({ extractedImages: ... })` paths and ensure they merge through the latest `this.data.extractedImages` or an equivalent durable per-result state map keyed by stable result identity. Preserve immediate result display, result ordering, batch/single-image processing, source-image indexes, `新任务` album-write cleanup decisions, local-cache cleanup, manual single-result save, all-download timeout/retry/failure-skip behavior, 3-active background predownload, failed-download non-requeue behavior, and Backend/API contracts. |
| SAVE-REV-002 | Reviewer | pending | Review save-state refresh durability after Frontend handoff. | Review only; `docs/agents/handoffs/2026-06-23-reviewer-save-state-refresh.md` | Reviewer must verify the actual `SAVE-FE-002` diff and handoff: after a result is marked album-written, later status refreshes, newly received results, batch progress updates, completion paths, and predownload callbacks cannot downgrade it from green-check `album`; downloaded local-path results cannot reset to yellow when local path is known; no stale `baseImages`/`collectedImages`/accumulated array path overwrites current state; `新任务` still uses album-written truth; source-image indexes and result ordering remain correct; all-download and manual save still use bounded retry/timeout/failure-skip behavior; no Backend/API changes. Required checks include `node --check wechat-miniprogram/pages/index/index.js`, `git diff --check`, and targeted mocks covering single-image polling refresh, batch polling refresh, finishing batch with stale accumulated arrays, background predownload after manual album save, downloaded-local refresh persistence, and `新任务` cleanup decisions after refresh. |
...
- None. The save-state refresh regression has concrete Frontend and Reviewer owners; Backend remains out of scope unless Frontend finds a concrete API blocker.
