...
| PICK-FE-001 | Frontend | done | Fix real WeChat image picker failures before upload starts. | `wechat-miniprogram/pages/index/index.js`, `wechat-miniprogram/pages/index/index.wxml` if needed, `docs/agents/handoffs/2026-06-21-frontend-image-picker.md` | Frontend commit `506cf46` switches image-only selection to `wx.chooseImage` with `wx.chooseMedia` only as an unavailable-API fallback, normalizes `tempFilePaths`, `tempFiles[].tempFilePath`, and `tempFiles[].path` into the existing selected-image model, preserves max 9 images and add-more behavior, treats user cancel as a no-op, logs raw non-cancel picker errors, and shows a concise failure toast for real picker failures. |
| PICK-REV-001 | Reviewer | done | Review the image-picker fix after Frontend handoff is available. | Review only; `docs/agents/handoffs/2026-06-21-reviewer-image-picker.md` | Reviewer verdict: approved. Reviewer confirmed the diff is Frontend-scoped, `wx.chooseImage` is preferred, fallback/normalization/cancel/failure handling works, adding after existing images caps at 9 while preserving current preview and per-image defaults, main-sync tab/calendar/`izaya7-map` routes remain intact, and upload/process/cancel/status/completed-result contracts were not touched. Checks passed: `node --check`, `git diff --check`, PM patch-id comparison, and targeted picker mocks. |
