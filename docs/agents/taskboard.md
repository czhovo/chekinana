...
| Frontend | `C:\Users\20888\Desktop\chekinana-frontend` | `codex/frontend-next` | `ASSET-FE-002` pending after PM taskboard sync |
| Backend | `C:\Users\20888\Desktop\chekinana-backend` | `codex/backend-next` | `ASSET-BE-002` pending after Frontend handoff, with parallel public asset URL verification allowed |
| Reviewer | `C:\Users\20888\Desktop\chekinana-reviewer` | `codex/reviewer-next` | `ASSET-REV-002` pending after Frontend and Backend handoffs |
...
| ASSET-FE-002 | Frontend | pending | Diagnose and fix the real-WeChat lianliankan asset loading failure and reset/retry recovery path. | `wechat-miniprogram/pages/lianliankan/**`, `wechat-miniprogram/workers/**` if asset path contracts need changes, optional mini-program asset-cache utility, `docs/agents/handoffs/2026-06-25-frontend-lianliankan-asset-load-fix.md` | In real mini-program semantics, first entry must either load all 14 remote images and start the board with valid local paths, or show a retryable error with the raw/sanitized failing API and URL. Reset/retry must clear stale/partial manifest, path mapping, and missing saved-file cache so a subsequent attempt redownloads correctly. The page must not mark assets ready if any required local file is missing or any download/save failed. Preserve settings entry, auth behavior, board generation, workers, and no bundled PNG fallback. The handoff must include the actual failure cause found, exact changed behavior, and targeted mock or simulator checks for first entry failure, reset/retry, partial cache, and successful cache reuse. |
| ASSET-BE-002 | Backend | pending | Verify and, if needed, fix the Cloudflare Pages public asset deployment for the lianliankan manifest and 14 PNGs. | `cloudflare-pages/**` or the static asset source created by `ASSET-BE-001`, deploy docs/scripts if needed, Cloudflare Pages config notes, `docs/agents/handoffs/2026-06-25-backend-lianliankan-public-assets.md` | Public URLs under `https://chekinana.top/assets/lianliankan/v1/` must return the expected manifest and all 14 PNG files with successful status and usable content types from outside the local file tree. Verify the exact manifest URL and all image URLs, and record status/content-type/hash or byte-size evidence in the handoff. Explicitly confirm that binding Cloudflare Pages to `chekinana.top` does not affect the existing `api.chekinana.top/*` Worker route. If the root domain/Page binding, DNS, build output directory, headers, or asset path is wrong, fix only the Cloudflare Pages/static asset path needed for this contract. Do not enable R2, do not use Alibaba OSS, and do not touch `api.chekinana.top`, the existing Worker route, Flask backend, RunPod startup, extraction APIs, auth/token flow, result APIs, or contact route. |
| ASSET-REV-002 | Reviewer | pending | Re-review the lianliankan remote asset fix after Frontend and Backend handoffs are available. | Review only; `docs/agents/handoffs/2026-06-25-reviewer-lianliankan-asset-load-fix.md` | Reviewer must not approve without verifying both sides of the failure: public `https://chekinana.top/assets/lianliankan/v1/manifest.json` and all 14 image URLs are reachable with expected content types, and the mini-program asset loader handles first entry, successful cache reuse, partial/missing cache, reset/retry after failure, manifest/version mismatch, and visible error state. Required checks include confirming `api.chekinana.top/*` still routes to the existing Worker/API path after Pages binding, `node --check` for changed mini-program JS/worker JS, manifest/static asset validation, targeted mocks or simulator checks for the user-reported reset-still-fails path, `git diff --check`, and explicit confirmation that RunPod, extraction APIs, auth flow, settings entry, and package-local PNG removal behavior are not regressed. |
...
- None. Publish follow-up tasks in Frontend, Backend, Reviewer order: `ASSET-FE-002`, `ASSET-BE-002`, then `ASSET-REV-002`.
